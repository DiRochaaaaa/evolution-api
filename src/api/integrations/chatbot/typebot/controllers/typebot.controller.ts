...
export class TypebotController extends BaseChatbotController<TypebotModel, TypebotDto> {
  ...
  protected async processBot(
    ...
    await this.typebotService.processTypebot(
      ...
      settings?.expire ?? 300,
      bot.typebot,
      settings?.keywordFinish ?? 'bye',
      settings?.delayMessage ?? 1000,
      settings?.unknownMessage ?? 'Sorry, I dont understand',
      settings?.listeningFromMe ?? true,
      settings?.stopBotFromMe ?? true,
